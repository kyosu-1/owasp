...
1. 「●●でログイン」しようとしたリソースオーナーのアクセストークン を取得する
2. クセストークンを使ってプロフィールAPIなどからユーザー識別子を取得
3. ユーザー識別子を用いて認証

1から3までの一連の流れによって認証が成立するが、2と3のようなユーザー識別子を含むリソースオーナーの情報のやりとりはOAuth2.0では定義されていない。
ここは拡張次第で実装は可能だが、OAuth2.0の枠を超えており、また安全性が独自の実装に委ねられる。

そのためOAuth認証は一般に推奨されない。

そこで、OAuth2.0を認証に拡張した**OpenID Connect**が生まれた。
...
と言った、クライアントが要求したリソースオーナーのプロフィール情報を含めることが可能。

### OpenID Connectのフロー

全体的なフローはOAuth2.0とほぼ同じで、基本的にアクセストークンがIDトークンに置き換わる形。
...
* Oauth2.0
  ...
  * アクセストークンを発行しクライアントに権限の委譲を行う
* OpenID Connect
  * Oauth2.0を拡張した認証プロトコル
  * 複数サービスでID連携を行うことでSSOも実現可能
